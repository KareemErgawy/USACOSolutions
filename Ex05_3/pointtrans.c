...

{
	...
	/* get list of optional function arguments */va_start(arguments,lineformat);
	...
	/* close argument list */va_end(arguments);
...

{
	...
	for (i = 1; i <= nx; i++) {
		for (j = 1; j <= ny; j++) {
			g[(int) u[i][j]] = a + ((b - a) * ((u[i][j] - min) / factor));
		}
	}
...

{
	 ...
	 */
	for (k = 0; k < 256; k++) {
		g[k] = 255.0f * pow(k / 255.0f, 1.0f / gamma);
	}
