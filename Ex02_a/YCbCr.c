...
for (i=1;i<=nx;i++)
  for (j=1;j<=ny;j++)
	 ...
	 */
	  ...
	  u_YCbCr[0][i][j] = (0.299f*u_RGB[0][i][j]) + (0.587f*u_RGB[1][i][j]) + (0.114f*u_RGB[2][i][j]);
	  u_YCbCr[1][i][j] = 127.5f + (-0.169f*u_RGB[0][i][j]) + (-0.331f*u_RGB[1][i][j]) + (0.5f*u_RGB[2][i][j]);
	  u_YCbCr[2][i][j] = 127.5f + (0.5f*u_RGB[0][i][j]) + (-0.419f*u_RGB[1][i][j]) + (-0.081f*u_RGB[2][i][j]);
      }
