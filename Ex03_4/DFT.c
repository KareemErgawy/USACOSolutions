...
for (i=0; i<nx; i++)
 for (j=0; j<ny; j++)
     ...
     */
	 // Here a Gaussian law-pass filter was applied to get rid of the high frequency noise components
	 float cutoffFreqSq = 32 * 32;
	 float freqContribution = expf(-((powf(i - centre_x, 2)/(2.0f*cutoffFreqSq)) + (powf(j - centre_y, 2)/(2.0f*cutoffFreqSq))));
	 ur[i][j] = ur[i][j] * freqContribution;
	 ui[i][j] = ui[i][j] * freqContribution;
...
{
char   in[80] = "smoke.pgm";               /* for reading data */
char   out1[80] = "smoke_out_log.pgm";             /* for reading data */
char   out2[80] = "smoke_out_back.pgm";             /* for reading data */
...
alloc_matrix (&w,  nx, ny);
alloc_matrix (&m,  nx, ny);
for (j=0; j<ny; j++)
 for (i=0; i<nx; i++)
     ui[i][j] = 0.0;
...
disalloc_matrix (w,  nx, ny);
disalloc_matrix (m,  nx, ny);
